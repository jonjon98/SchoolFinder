...
Future<List<School3>> _getSchools() async{
  ...
  List<School3> AllSchoolList3 = [];

  for(var s in cca_Data){
    School3 school = School3(s['school_name'], s['school_section'], s['cca_grouping_desc'],
        s['cca_customized_name'], s['cca_generic_name']);
    AllSchoolList3.add(school);
  }
  int schoolNo = AllSchoolList3.length;
  print('Total no. of Schools in this list: $schoolNo');
  //print(AllSchoolList3[344].school_name);
  //print(AllSchoolList3[344].cca_generic_name);
  return AllSchoolList3;

}
  //Function(s) to retrieve info/filter schools-----------------------------------------------------------------------------------------------------------------------------------------
  //Retrieve CCA information of a particular school
    Future<List<String>> getCCAInfo(school_name, String x) async {
    List<School3> AllSchoolList3 = await _getSchools();
    List<String> schoolSectionList = [];  //may contain duplicated "Primary"/"Secondary"/"Tertiary" but most likely won't use this
    List<String> ccaGroupList = [];       //may contain duplicated info but most likely won't use this
    List<String> ccaCustList = [];        //may contain "na" if the school does not have a customised name for the CCA
    List<String> ccaGenList = [];         //CCA list to be used

    switch (x){
      case "school_section":
        for (int i = 0; i < AllSchoolList3.length; i++){
          if (AllSchoolList3[i].school_name == school_name)
          {
            schoolSectionList.add(AllSchoolList3[i].school_section);
            // return(schoolSectionList);
          }
        }
        print(schoolSectionList);
        break;
      case "cca_grouping_desc":
        for (int i = 0; i < AllSchoolList3.length; i++){
          if (AllSchoolList3[i].school_name == school_name)
          {
            ccaGroupList.add(AllSchoolList3[i].cca_grouping_desc);
            // return(ccaGroupList);
          }
        }
        print(ccaGroupList);
        break;
      case "cca_customized_name":
        for (int i = 0; i < AllSchoolList3.length; i++){
          if (AllSchoolList3[i].school_name == school_name)
          {
            ccaCustList.add(AllSchoolList3[i].cca_customized_name);
            // return(ccaCustList);
          }
        }
        print(ccaCustList);
        break;
      case "cca_generic_name":
        for (int i = 0; i < AllSchoolList3.length; i++){
          if (AllSchoolList3[i].school_name == school_name)
          {
            ccaGenList.add(AllSchoolList3[i].cca_generic_name);
            // return(ccaGenList);
          }
        }
        return ccaGenList;
  ...
  //Filter Schools by cca(s), return list of schools
  Future<List<String>> _getSchoolByCCA(cca) async {
    List<School3> AllSchoolList3 = await _getSchools();
    List<String> schoolsByCCAList = [];

    for (int i = 0; i < AllSchoolList3.length; i++){
      if (AllSchoolList3[i].cca_generic_name == cca)
      {
        schoolsByCCAList.add(AllSchoolList3[i].school_name);
      }
    }
    return(schoolsByCCAList);
    //test output
  }
...
class School3 {
  ...
  String cca_grouping_desc;
  String cca_customized_name;
  String cca_generic_name;

  School3(this.school_name, this.school_section, this.cca_grouping_desc,
      this.cca_customized_name, this.cca_generic_name);
}

class SchoolList3 extends StatelessWidget {
  Future<List<School3>> fetchSchools() async{
    Future<List<School3>> allSchool = _getSchools();
    return allSchool;
  }

  @override
  Widget build(BuildContext context) {
    Future<List<School3>> allSchools = fetchSchools();
    return Scaffold(
      appBar: AppBar(
        title: Text('Search Schools'),
        actions: <Widget>[
          Container(
              child: FutureBuilder<List<School3>>(
                  future: allSchools,
...
class DataSearch extends SearchDelegate<String> {
  ...
  List<School3> allSchools;

  DataSearch(this.allSchools);

  List<String> convertList(List<School3> allSchools) {
